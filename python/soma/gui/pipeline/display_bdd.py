...
class DisplayBDD(QtGui.QDialog):
    def __init__(self,process):
        super(DisplayBDD, self).__init__()
        ...
        #TOFIX
        self.process_name='morphologistSimp.SimplifiedMorphologist'
        self.add_header()
    ...
    def add_header(self):
        ...
        self.table.setHorizontalHeaderItem(col,QtGui.QTableWidgetItem('JSONFILE'))
        col = self.table.columnCount()
        self.table.setColumnCount( col + 1 )
        self.table.setHorizontalHeaderItem(col,QtGui.QTableWidgetItem('Subject'))
        for name,trait in self.process.user_traits().iteritems():
            if trait.is_trait_type( File ) is True:
                ...
                #and trait.hidden is not True:
                    col = self.table.columnCount()
                    self.table.setColumnCount( col + 1 )
                    self.table.setHorizontalHeaderItem(col,QtGui.QTableWidgetItem(name))
    ...
    def add_subjects_on_table(self):
        ...
        for key in self.sub:
            ...
            self.dico_btn_launch_json[btn_launch_json]=self.sub[key]['jsonfile']
            ...
            #self.table.setHorizontalHeaderItem(1,QtGui.QTableWidgetItem('Subject'))
            col=2
            for key2 in self.sub[key]['output']:
                #col = self.table.columnCount()
                #self.table.setColumnCount( col + 1 )
                #print 'chemin',self.sub[key]['output'][key2]
                #print 'res', os.path.exists(self.sub[key]['output'][key2])
                if os.path.exists(self.sub[key]['output'][key2]) is True:
                    self.table.setItem( row, col, QtGui.QTableWidgetItem())
                    self.table.item(row, col).setBackground(QtGui.QColor(50,205,50))
                else:
                    self.table.setItem( row, col, QtGui.QTableWidgetItem())
                    self.table.item(row, col).setBackground(QtGui.QColor(238,64,0))
        
                col=col+1
                #if self.first_subject==0:
                    #print 'go header'
                    #self.table.setHorizontalHeaderItem(col,QtGui.QTableWidgetItem(key2))
    ...
    def check_subjets_not_used(self):
        ...
        #file_selection.select( self.Study.input_fom, 'unused', [ 'unused' ], self.Study.input_directory)
        selector_class = file_selection.find_selector(self.Study.input_fom, 'unused', [ 'unused' ])
        if selector_class:
        ...
        for ele in a.attributes:
            if ele['subject'] not in self.list_subjects:
                ...
                self.table.setItem( row, 1, QtGui.QTableWidgetItem(ele['subject']))
                for num_col in range(2,self.table.columnCount()):
                    self.table.setItem( row, num_col, QtGui.QTableWidgetItem())
                    self.table.item(row, num_col).setBackground(QtGui.QColor(190,190,190))
        
        #selection = file_selection.select( fom, 'unused', [ 'unused' ], sys.argv[ 1 ] )
        #selection = file_selection.select( self.Study.input_fom, 'unused', [ 'unused' ], direct)        
        
                            #file_selection = FileAttributeSelection()
        #selection = file_selection.select( self.Study.input_fom, 'unused', [ 'unused' ], self.Study.input_directory)

    def on_launch_json(self):
        sender = self.sender()
        QtGui.QDesktopServices.openUrl(QtCore.QUrl(self.dico_btn_launch_json[sender]))
    ...
    def get_directories(self):
        ...
        for path in os.listdir(self.Study.output_directory):
            if not os.path.isfile(os.path.join(self.Study.output_directory,path)):
    ...
    def load(self,json_file):
        try:
            with open(json_file, 'r') as json_data:
                ...
                self.sub[data['run']['attributes']['subject']]=collections.OrderedDict()
                #self.sub[data['run']['attributes']['subject']]['output']=data['run']['output']
                self.sub[data['run']['attributes']['subject']]['output']=data['run']['output']
                self.sub[data['run']['attributes']['subject']]['jsonfile']=json_file

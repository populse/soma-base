...
class IterationGui(QtGui.QWizardPage):
    """Class who created Interface for execute a process on many subjects"""
    def __init__(self):     
        super(IterationGui, self).__init__() 
	...
	for key in sorted(self.pwd.attributes):
	    ...
	    self.table.setHorizontalHeaderItem(num_col,QtGui.QTableWidgetItem(key))
	    self.dico_header[key]=num_col
	    num_col=num_col+1
	self.table.setHorizontalHeaderItem(num_col,QtGui.QTableWidgetItem('delete'))
	    
    """ Add subject on database and create Attributs and SimpMorpho objects for each """    
	...
	for i in range(0,len(selection)):
		...
		self.pwd=ProcessWithFom(self.get_object_process())
		...
		if self.first_subject_add==0:
		    ...
		    self.first_subject_add=1
                ...
                #Just list to have value of attributes and to check if something wrong
		liste=[]
		for key in sorted(self.pwd.attributes):    
		    liste.append(self.pwd.attributes[key])
		#If only filename probablye error in the filename (TO DO: better to get the raise ValueError in the fom if subject not good)
		if len(liste)==0:
		    print "no attributs something wrong?"
		...
		for a in range(0,len(liste)): 
		    self.table.setItem(len(self.list_subjects_selected)-1, a, QtGui.QTableWidgetItem(liste[a]))	
		btn_delete=QtGui.QPushButton('Delete')    
		self.dico_btn_delete[btn_delete]=len(self.list_subjects_selected)-1	
		btn_delete.clicked.connect(self.on_del_subjects) 
		self.table.setCellWidget(len(self.list_subjects_selected)-1,len(liste), btn_delete)
   
   
    def on_del_subjects(self):
        sender = self.sender()
	the_item=self.table.item(self.dico_btn_delete[sender], self.dico_header['subject'])
	the_i=the_item.text()
    ...
    def get_object_process(self):
        ...
        if Study.get_instance().process=='morphologistSimp.SimplifiedMorphologist':
	    import morphologistSimp
	    obj=morphologistSimp.SimplifiedMorphologist()
	    return obj
...
class ChoiceParameters(QtGui.QWizardPage):
	...
	self.vbox=QtGui.QVBoxLayout()
	self.table=QtGui.QTableWidget(1,1)
	self.table.setHorizontalHeaderItem(0,QtGui.QTableWidgetItem('Subjects'))
    ...
    def trait_parameter_to_widget(self,trait,type_trait,all_value,default_value):
        if type_trait=='Enum':
	    wid=QtGui.QComboBox()	 
	    for val in all_value:
		wid.addItem(val)
		#wid.currentIndexChanged.connect(self.parameter_changed)
	    self.connect(wid,QtCore.SIGNAL("currentIndexChanged(const QString &)"), partial(self.combo_parameter_changed,trait))
		
	    #return wid
	elif type_trait=='Bool':
	    wid=QtGui.QCheckBox()
	    wid.setChecked(default_value)
	    self.connect(wid,QtCore.SIGNAL("stateChanged(int)"), partial(self.checkbox_parameter_changed,trait))

...
class CATIScanSelector( QDialog ):
  ...
  glob = ( '*', 'CONVERTED', '*', '*', '*' )
  path_re = ( '(?P<study>.*)', '(?P<test>CONVERTED)', '(?P<center>.*)', '(?P<subject>.*)', '(?P<time_point>.*)' )
  ...
  def reset( self, directory ):
    '''
    Clear the selection dialog and parse directory to extract selectable attributes.
    '''
    self.table.clear()
    ...
    time_points = set()
    subjects = set()
    path_re = re.compile( os.path.join( os.path.join( directory, *self.path_re ) ) )
    self.attributes = []
    for i in glob( os.path.join( directory, *self.glob ) ):
      ...
      if match:
        ...
        for i in xrange( len( self.match_groups ) ):
          self.table.setItem( row, i, QTableWidgetItem( match.group( self.match_groups[ i ] ) ) )
  ...
  def select( self ):
    ...
    if self.exec_() == self.Accepted:
      result = []
      for range in self.table.selectedRanges():
        result.extend( self.attributes[ range.topRow() : range.bottomRow() + 1 ] )
      return result
    else:
      return False
...
class BrainVISAScanSelector( CATIScanSelector ):
  ...
  glob = ( '*', '*', 't1mri', '*' )
  path_re = ( '(?P<center>.*)', '(?P<subject>.*)', 't1mri', '(?P<acquisition>.*)' )
  match_groups = [ 'center', 'subject', 'acquisition' ]

  
class FileAttributeSelection( object ):
  '''
  A FileAttributeSelection is used to select multiple files according to their
  attributes as defined in a given FOM.
  ...
  def __init__( self ):
    self._selector_rules = [ ( re.compile( i[0] ), re.compile( i[1] ), j ) for i,j in self._selectors.iteritems() ]
  
  
  def find_selector( self, fom_name, process_name, parameters ):
    '''
    Find the selector class allowing to select multiple files for a given process.
    fom : FileOrganisationModel instance
    ...
    for fom_re, process_re, selector_class in self._selector_rules:
      #if fom_re.match( fom.fom_names[0] ) and process_re.match( process_name ):
